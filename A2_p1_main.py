...
noncoding_intervals = load_noncoding_intervals("UCSC_genome_table_d.csv")
noncoding_signals = []

for s, e in noncoding_intervals:
    signal = interval_average_signal([(s, e)], gene_expr)
    if signal > 0:
        noncoding_signals.append((signal, s, e))

noncoding_signals.sort(reverse=True)
print("Top 5 expressed non-coding regions:")
for signal, s, e in noncoding_signals[:5]:
    print(f"  {s}-{e}, signal={signal:.3f}")